...
class PhotoEmailService:
	...
	def save_unread_email_photos(self):
		...
		print("{0} unprocessed emails received.".format(emails))
		...
		for i in range(emails):
			...
			#--------------------------------#
			sender_address = parse_from_address(str_message['From'])
			'''
			cur = db.cursor()
			...
			for part in str_message.walk():
				# If either of these conditions are true, then the email
				# does not have attachments, so we skip it and move to the next one
				if (part.get_content_maintype() == 'multipart'
				or part.get('Content-Disposition') is None):
					# ERROR: Email does not have attachments
					continue
				...
				send_email(sender_address, EMAIL_SUBJECT_SUCCESS, EMAIL_BODY_SUCCESS)

		# Prevents messages from being removed from Gmail
		import sys
		sys.exit(0)
